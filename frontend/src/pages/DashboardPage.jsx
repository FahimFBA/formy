  ...
  return (
    <Layout>
      <div className="flex items-center justify-between gap-3">
        <h1 className="text-2xl font-semibold text-slate-950">Your forms</h1>
        <button
      ...
      {loading ? (
        ...
        <p className="mt-6 text-sm text-slate-600">No forms yet. Create your first one.</p>
      ) : (
      ...
      {!loading && count > 0 ? (
        <div className="mt-6 flex items-center justify-between gap-3">
          <p className="text-sm text-slate-600">
            Page {page} of {pageCount} &middot; {count} form{count === 1 ? "" : "s"}
          </p>
      ...
      <ConfirmDialog
        ...
        title="Delete form?"
        description={pendingDelete ? `"${pendingDelete.name}" and all its submissions will be permanently deleted. This can't be undone.` : ""}
        confirmLabel="Delete"
        danger
