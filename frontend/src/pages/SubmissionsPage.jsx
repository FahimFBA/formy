  ...
  return (
    <Layout>
      ...
      <div className="mt-3 flex items-center justify-between gap-3">
        <h1 className="text-2xl font-semibold text-slate-950 dark:text-white">{t("nav_submissions")}</h1>
      ...
      {loading ? (
        <p className="mt-4 text-sm text-slate-600 dark:text-slate-400">{t("msg_loading")}</p>
      ) : null}

      {!loading && submissions.length === 0 ? (
        <p className="mt-4 text-sm text-slate-600 dark:text-slate-400">{t("msg_no_submissions")}</p>
      ) : null}
      ...
      <div className="mt-4 space-y-3">
        {submissions.map((submission) => (
          <div
            key={submission.id}
            className="rounded-md border border-slate-200 bg-white p-4 shadow-panel dark:border-slate-800 dark:bg-slate-900"
          >
            <p className="text-xs text-slate-500 dark:text-slate-400">
              {new Date(submission.submitted_at).toLocaleString()} &middot; schema v{submission.schema_version}
            </p>
            <pre className="mt-2 overflow-auto rounded-md bg-slate-50 p-3 text-xs leading-6 text-slate-800 dark:bg-slate-950 dark:text-slate-300">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {submission.attachments.map((attachment) => (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 rounded-md border border-slate-300 bg-white px-2.5 py-1 text-xs font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800"
                  >
                    <Paperclip size={12} />
                    {attachment.filename}
                  </button>
